...
$lead->status = $lead->status == 'Sold' ? 'CONNECTION PAPERWORK NOT STARTED' : $lead->status;
...
$connectionApplicationNumberValue = $leadMeta['connection_application_number'] ?? '';
$connectionSubmittedDateValue = $leadMeta['connection_submitted_date'] ?? '';
$connectionApprovalDateValue = $leadMeta['connection_approval_date'] ?? '';
$connectionNotesValue = $leadMeta['connection_notes'] ?? '';
$vicWorkflowStatuses = [
...
$currentVicWorkflowStatus = !empty($lead->status)
    ? $lead->status
    : 'CONNECTION PAPERWORK NOT STARTED';
...
<div class="rk-overview-wrapper">
    ...
    <div class="rk-overview-header">
        <div class="rk-overview-title-wrap">
            ...
            <div>
                <span class="rk-overview-kicker"><i class="ph ph-plugs-connected"></i> Connection Paperwork</span>
        ...
        <div class="rk-column-stack rk-column-stack-full">
            <div class="row g-3">
                ...
                <div class="col-sm-6">
                    <div class="rk-summary-card h-100">
                <div class="rk-summary-header">
                    <strong><i class="ph ph-plugs-connected"></i> Connection Paperwork Details</strong>
                </div>
                ...
                <div class="rk-summary-content">
                    <div class="rk-summary-row">
                        <div>
	                        ...
	                        <div>
	                            ...
	                            <p>{{ $connectionApplicationNumberValue ?: '-' }}</p>
	                        </div>

	                        <div>
	                            <label>Submitted Date</label>
	                            <p>{{ $connectionSubmittedDateValue ?: '-' }}</p>
	                        </div>
                    </div>
                    ...
                    <div class="rk-summary-row">
	                        <div>
	                            ...
	                            <p>{{ $connectionApprovalDateValue ?: '-' }}</p>
	                        </div>
...
<div class="offcanvas offcanvas-end rk-vic-modal" id="vicWorkflowModal" tabindex="-1" aria-labelledby="vicWorkflowModalLabel">
            <div class="offcanvas-header rk-vic-modal-header">
                <div>
                    <span class="rk-vic-offcanvas-kicker"><i class="ph ph-sparkle"></i> Workflow</span>
                    <h4 class="offcanvas-title" id="vicWorkflowModalLabel">Connection Paperwork - Job #{{ $lead->id }}</h4>
            ...
            <div class="offcanvas-body rk-vic-modal-body">
                ...
                <div class="rk-vic-pane active" data-vic-pane="action">
                    <div class="rk-vic-info-card">
                        ...
                        <div>
                            <h5>Connection Paperwork Action Data</h5>
                            <p>Update paperwork status, capture application details, and run the next action from this popup.</p>
                        </div>
                    </div>

                    <h5 class="mb-3"><i class="ph ph-plugs-connected"></i> Connection Progress</h5>
                    <div class="row g-3">
                        <div class="col-md-12">
                            <div class="rk-vic-field-group mb-0">
                                <label for="connectionStatus">Status</label>
                                <select id="connectionStatus" class="form-select rk-vic-select-focus">
                        ...
                        <div class="col-md-4">
                            <div class="rk-vic-info-card h-100 mb-0">
	                                <div class="rk-vic-info-icon"><i class="ph ph-identification-card"></i></div>
	                                <div>
                        ...
                        <div class="col-md-4">
                            <div class="rk-vic-info-card h-100 mb-0">
	                                <div class="rk-vic-info-icon"><i class="ph ph-calendar-check"></i></div>
	                                <div>
	                                    <h5>Approval Date</h5>
	                                    <p>{{ $connectionApprovalDateValue ?: '-' }}</p>
                                </div>
	                    ...
	                    <h5 class="mb-3"><i class="ph ph-clipboard-text"></i> Application Data</h5>
	                    <div class="row g-3">
	                        <div class="col-md-6">
	                            <div class="rk-vic-field-group mb-0">
	                                <label for="connectionApplicationNumber">Application Number</label>
	                                <input type="text" id="connectionApplicationNumber" class="form-control" value="{{ $connectionApplicationNumberValue }}" placeholder="Enter connection application number">
	                            </div>
	                        </div>
	                        <div class="col-md-6">
	                            <div class="rk-vic-field-group mb-0">
	                                <label for="connectionSubmittedDate">Submitted Date</label>
	                                <input type="date" id="connectionSubmittedDate" class="form-control" value="{{ $connectionSubmittedDateValue }}">
	                            </div>
	                        </div>
	                        <div class="col-md-12">
	                            <div class="rk-vic-field-group mb-0">
	                                <label for="connectionApprovalDate">Approval Date</label>
	                                <input type="date" id="connectionApprovalDate" class="form-control" value="{{ $connectionApprovalDateValue }}">
	                            </div>
	                        </div>
	                        <div class="col-md-12">
	                            <div class="rk-vic-field-group mb-0">
	                                <label for="connectionNotes">Notes</label>
	                                <textarea id="connectionNotes" class="form-control rk-vic-textarea" rows="3" placeholder="Add connection paperwork notes">{{ $connectionNotesValue }}</textarea>
	                            </div>
	                        </div>
	                    </div>
                ...
                <div class="rk-vic-pane" data-vic-pane="uploads">
                    <div class="rk-vic-info-card">
                        ...
                        <div>
	                            <h5>Uploads Data</h5>
	                            <p>Upload and review connection paperwork, network forms, approvals, and supporting documents.</p>
                        </div>
                    </div>

                    <div class="rk-vic-field-group">
	                        <label for="connectionFileInput">Upload Connection File</label>
	                        <div class="d-flex flex-wrap align-items-center gap-2">
	                            <input type="file" id="connectionFileInput" class="form-control" style="max-width: 360px;">
	                            <button type="button" class="btn btn-primary" id="connectionUploadBtn">
	                                <i class="ph ph-upload-simple"></i> Upload
	                            </button>
	                        </div>
	                        <small class="text-muted connection-upload-status">No file selected</small>
                    </div>

	                    <ul class="rk-attach-list mt-3" id="connectionFileList">
                        @forelse($lead->images as $file)
                            @php
                            $fileUrl = preg_replace('/^admin\//', '', $file->image_path);
                            $fullUrl = asset($fileUrl);
                ...
                <div class="rk-vic-pane" data-vic-pane="timeline">
                    <div class="rk-vic-info-card">
                        ...
                        <div>
                            <h5>Timeline</h5>
	                            <p>Review connection paperwork notes and add the next application update from this popup.</p>
                        </div>
                    </div>
                    ...
                    <form method="POST" action="{{ route('superadmin.noteStore') }}" class="mb-3">
                        ...
                        <div class="rk-vic-field-group mb-2">
	                            <label for="connectionTimelineNote">Add Timeline Note</label>
	                            <textarea id="connectionTimelineNote" name="note" class="form-control rk-vic-textarea" rows="3" placeholder="Add connection paperwork timeline update..." required></textarea>
                        </div>
            ...
            <div class="rk-vic-modal-footer">
                <button type="button" class="btn btn-light" data-bs-dismiss="offcanvas">Close</button>
	                <button type="button" class="btn btn-primary" id="saveVicWorkflowBtn">Save Connection Data</button>
            </div>
</div>
...
$(function() {
    ...
    $('.rk-vic-step').on('click', function() {
	    ...
	    $('#saveVicWorkflowBtn').on('click', function() {
	        const $btn = $(this);
	        const status = $('#connectionStatus').val();
	
	        if (!status) {
	            setVicPane('action');
	            showWorkflowAlert('warning', 'Required', 'Please select connection paperwork status.');
	            return;
	        }
	        ...
	        $.ajax({
	            url: '{{ route("superadmin.saveConnectionPaperworkMeta") }}',
	            type: 'POST',
	            data: {
	                _token: '{{ csrf_token() }}',
	                lead_id: leadId,
	                status: status,
	                application_number: $('#connectionApplicationNumber').val(),
	                submitted_date: $('#connectionSubmittedDate').val(),
	                approval_date: $('#connectionApprovalDate').val(),
	                notes: $('#connectionNotes').val()
	            },
	            success: function() {
	                reloadWithSuccess('Connection paperwork data saved successfully.');
	            },
	            error: function(xhr) {
	                showWorkflowAlert('error', 'Failed', xhr.responseJSON && xhr.responseJSON.message ? xhr.responseJSON.message : 'Failed to save connection paperwork data.');
	                $btn.prop('disabled', false).text('Save Connection Data');
	            }
	        });
	    });
	
	    $('#connectionFileInput').on('change', function() {
	        const file = this.files[0];
	        $('.connection-upload-status').text(file ? file.name : 'No file selected');
	    });
	
	    $('#connectionUploadBtn').on('click', function() {
	        const $btn = $(this);
	        const fileInput = $('#connectionFileInput')[0];
	        const file = fileInput.files[0];
	
	        if (!file) {
	            setVicPane('uploads');
	            showWorkflowAlert('warning', 'Required', 'Please choose a connection paperwork file to upload.');
	            return;
	        }
        ...
        const formData = new FormData();
	        ...
	        formData.append('status', $('#connectionStatus').val());
	        formData.append('upload_context', 'connection_paperwork');
        ...
        $.ajax({
            ...
            processData: false,
	            ...
	            success: function(res) {
	                $('#connectionFileList .rk-attach-empty').remove();
	                $('#connectionFileList').append(
	                    '<li class="rk-attach-item"><a href="' + res.file_url + '" target="_blank" class="rk-attach-link"><i class="ph ph-file"></i> ' + res.file_name + '</a></li>'
	                );
	                fileInput.value = '';
	                $('.connection-upload-status').text('Uploaded successfully');
	            },
	            error: function(xhr) {
	                showWorkflowAlert('error', 'Upload failed', xhr.responseJSON && xhr.responseJSON.message ? xhr.responseJSON.message : 'Failed to upload connection paperwork file.');
	            },
            complete: function() {
