...
$lead->status = $lead->status == 'Sold' ? 'SOLAR VIC CLAIM SUBMITTED' : $lead->status;
...
$vicPaymentClaimNumberValue = $leadMeta['vic_payment_claim_number'] ?? '';
$vicPaymentDateValue = $leadMeta['vic_payment_date'] ?? '';
$vicPaymentAmountValue = $leadMeta['vic_payment_amount'] ?? '';
$vicPaymentNotesValue = $leadMeta['vic_payment_notes'] ?? '';
$vicWorkflowStatuses = [
...
<div class="rk-overview-wrapper">
    ...
    <div class="rk-overview-header">
        <div class="rk-overview-title-wrap">
            ...
            <div>
                <span class="rk-overview-kicker"><i class="ph ph-sparkle"></i> Solar VIC Payment</span>
        ...
        <div class="rk-column-stack rk-column-stack-full">
            <div class="row g-3">
                ...
                <div class="col-sm-6">
                    <div class="rk-summary-card h-100">
                <div class="rk-summary-header">
                    <strong><i class="ph ph-sparkle"></i> Solar VIC Payment Details</strong>
                </div>
                ...
                <div class="rk-summary-content">
                    <div class="rk-summary-row">
                        <div>
	                        ...
	                        <div>
	                            ...
	                            <p>{{ $vicPaymentClaimNumberValue ?: '-' }}</p>
	                        </div>

	                        <div>
	                            <label>Payment Date</label>
	                            <p>{{ $vicPaymentDateValue ?: '-' }}</p>
	                        </div>
                    </div>
                    ...
                    <div class="rk-summary-row">
	                        <div>
	                            <label>Amount</label>
	                            <p>{{ $vicPaymentAmountValue !== '' ? '$'.$vicPaymentAmountValue : '-' }}</p>
	                        </div>
...
<div class="offcanvas offcanvas-end rk-vic-modal" id="vicWorkflowModal" tabindex="-1" aria-labelledby="vicWorkflowModalLabel">
            <div class="offcanvas-header rk-vic-modal-header">
                <div>
                    ...
                    <h4 class="offcanvas-title" id="vicWorkflowModalLabel">Solar VIC Payment - Job #{{ $lead->id }}</h4>
            ...
            <div class="offcanvas-body rk-vic-modal-body">
                ...
                <div class="rk-vic-pane active" data-vic-pane="action">
                    <div class="rk-vic-info-card">
                        ...
                        <div>
                            <h5>Solar VIC Payment Action Data</h5>
                            <p>Update payment status, capture claim details, and run the next action from this popup.</p>
                        </div>
                    </div>
                    ...
                    <div class="row g-3">
                        <div class="col-md-12">
                            <div class="rk-vic-field-group mb-0">
                                <label for="vicPaymentStatus">Status</label>
                                <select id="vicPaymentStatus" class="form-select rk-vic-select-focus">
                        ...
                        <div class="col-md-4">
                            <div class="rk-vic-info-card h-100 mb-0">
	                                <div class="rk-vic-info-icon"><i class="ph ph-currency-dollar"></i></div>
	                                <div>
	                                    <h5>Amount</h5>
	                                    <p>{{ $vicPaymentAmountValue !== '' ? '$'.$vicPaymentAmountValue : '-' }}</p>
                                </div>
	                    ...
	                    <h5 class="mb-3"><i class="ph ph-clipboard-text"></i> Claim Data</h5>
	                    <div class="row g-3">
	                        <div class="col-md-6">
	                            <div class="rk-vic-field-group mb-0">
	                                <label for="vicPaymentClaimNumber">Claim Number</label>
	                                <input type="text" id="vicPaymentClaimNumber" class="form-control" value="{{ $vicPaymentClaimNumberValue }}" placeholder="Enter Solar VIC claim number">
	                            </div>
	                        </div>
	                        <div class="col-md-6">
	                            <div class="rk-vic-field-group mb-0">
	                                <label for="vicPaymentDate">Payment Date</label>
	                                <input type="date" id="vicPaymentDate" class="form-control" value="{{ $vicPaymentDateValue }}">
	                            </div>
	                        </div>
	                        <div class="col-md-12">
	                            <div class="rk-vic-field-group mb-0">
	                                <label for="vicPaymentAmount">Payment Amount</label>
	                                <input type="number" min="0" step="0.01" id="vicPaymentAmount" class="form-control" value="{{ $vicPaymentAmountValue }}" placeholder="0.00">
	                            </div>
	                        </div>
	                        <div class="col-md-12">
	                            <div class="rk-vic-field-group mb-0">
	                                <label for="vicPaymentNotes">Notes</label>
	                                <textarea id="vicPaymentNotes" class="form-control rk-vic-textarea" rows="3" placeholder="Add Solar VIC payment notes">{{ $vicPaymentNotesValue }}</textarea>
	                            </div>
	                        </div>
	                    </div>
                ...
                <div class="rk-vic-pane" data-vic-pane="uploads">
                    <div class="rk-vic-info-card">
                        ...
                        <div>
	                            <h5>Uploads Data</h5>
	                            <p>Upload and review Solar VIC payment receipts, claim files, and supporting documents.</p>
                        </div>
                    </div>

                    <div class="rk-vic-field-group">
	                        <label for="vicPaymentFileInput">Upload Solar VIC Payment File</label>
	                        <div class="d-flex flex-wrap align-items-center gap-2">
	                            <input type="file" id="vicPaymentFileInput" class="form-control" style="max-width: 360px;">
	                            <button type="button" class="btn btn-primary" id="vicPaymentUploadBtn">
	                                <i class="ph ph-upload-simple"></i> Upload
	                            </button>
	                        </div>
	                        <small class="text-muted vic-payment-upload-status">No file selected</small>
                    </div>

	                    <ul class="rk-attach-list mt-3" id="vicPaymentFileList">
                        @forelse($lead->images as $file)
                            @php
                            $fileUrl = preg_replace('/^admin\//', '', $file->image_path);
                            $fullUrl = asset($fileUrl);
                ...
                <div class="rk-vic-pane" data-vic-pane="timeline">
                    <div class="rk-vic-info-card">
                        ...
                        <div>
                            <h5>Timeline</h5>
	                            <p>Review Solar VIC payment notes and add the next claim update from this popup.</p>
                        </div>
                    </div>
                    ...
                    <form method="POST" action="{{ route('superadmin.noteStore') }}" class="mb-3">
                        ...
                        <div class="rk-vic-field-group mb-2">
	                            <label for="vicPaymentTimelineNote">Add Timeline Note</label>
	                            <textarea id="vicPaymentTimelineNote" name="note" class="form-control rk-vic-textarea" rows="3" placeholder="Add Solar VIC payment timeline update..." required></textarea>
                        </div>
            ...
            <div class="rk-vic-modal-footer">
                <button type="button" class="btn btn-light" data-bs-dismiss="offcanvas">Close</button>
	                <button type="button" class="btn btn-primary" id="saveVicWorkflowBtn">Save VIC Payment Data</button>
            </div>
</div>
...
$(function() {
    ...
    $('.rk-vic-step').on('click', function() {
	    ...
	    $('#saveVicWorkflowBtn').on('click', function() {
	        const $btn = $(this);
	        const status = $('#vicPaymentStatus').val();
	
	        if (!status) {
	            setVicPane('action');
	            showWorkflowAlert('warning', 'Required', 'Please select Solar VIC payment status.');
	            return;
	        }
	        ...
	        $.ajax({
	            url: '{{ route("superadmin.saveVicPaymentMeta") }}',
	            type: 'POST',
	            data: {
	                _token: '{{ csrf_token() }}',
	                lead_id: leadId,
	                status: status,
	                claim_number: $('#vicPaymentClaimNumber').val(),
	                payment_date: $('#vicPaymentDate').val(),
	                payment_amount: $('#vicPaymentAmount').val(),
	                notes: $('#vicPaymentNotes').val()
	            },
	            success: function() {
	                reloadWithSuccess('Solar VIC payment data saved successfully.');
	            },
	            error: function(xhr) {
	                showWorkflowAlert('error', 'Failed', xhr.responseJSON && xhr.responseJSON.message ? xhr.responseJSON.message : 'Failed to save Solar VIC payment data.');
	                $btn.prop('disabled', false).text('Save VIC Payment Data');
	            }
	        });
	    });
	
	    $('#vicPaymentFileInput').on('change', function() {
	        const file = this.files[0];
	        $('.vic-payment-upload-status').text(file ? file.name : 'No file selected');
	    });
	
	    $('#vicPaymentUploadBtn').on('click', function() {
	        const $btn = $(this);
	        const fileInput = $('#vicPaymentFileInput')[0];
	        const file = fileInput.files[0];
	
	        if (!file) {
	            setVicPane('uploads');
	            showWorkflowAlert('warning', 'Required', 'Please choose a Solar VIC payment file to upload.');
	            return;
	        }
        ...
        const formData = new FormData();
	        ...
	        formData.append('status', $('#vicPaymentStatus').val());
	        formData.append('upload_context', 'vic_payment');
        ...
        $.ajax({
            ...
            processData: false,
	            ...
	            success: function(res) {
	                $('#vicPaymentFileList .rk-attach-empty').remove();
	                $('#vicPaymentFileList').append(
	                    '<li class="rk-attach-item"><a href="' + res.file_url + '" target="_blank" class="rk-attach-link"><i class="ph ph-file"></i> ' + res.file_name + '</a></li>'
	                );
	                fileInput.value = '';
	                $('.vic-payment-upload-status').text('Uploaded successfully');
	            },
	            error: function(xhr) {
	                showWorkflowAlert('error', 'Upload failed', xhr.responseJSON && xhr.responseJSON.message ? xhr.responseJSON.message : 'Failed to upload Solar VIC payment file.');
	            },
            complete: function() {
