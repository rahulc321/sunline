...
$lead->status = $lead->status == 'Sold' ? 'STC CLAIM SUBMITTED' : $lead->status;
...
$stcPaymentClaimNumberValue = $leadMeta['stc_payment_claim_number'] ?? '';
$stcPaymentDateValue = $leadMeta['stc_payment_date'] ?? '';
$stcPaymentAmountValue = $leadMeta['stc_payment_amount'] ?? '';
$stcPaymentNotesValue = $leadMeta['stc_payment_notes'] ?? '';
$vicWorkflowStatuses = [
...
<div class="rk-overview-wrapper">
    ...
    <div class="rk-overview-header">
        <div class="rk-overview-title-wrap">
            ...
            <div>
                <span class="rk-overview-kicker"><i class="ph ph-certificate"></i> STCs Payment</span>
        ...
        <div class="rk-column-stack rk-column-stack-full">
            <div class="row g-3">
                ...
                <div class="col-sm-6">
                    <div class="rk-summary-card h-100">
                <div class="rk-summary-header">
                    <strong><i class="ph ph-certificate"></i> STCs Payment Details</strong>
                </div>
                ...
                <div class="rk-summary-content">
                    <div class="rk-summary-row">
                        <div>
	                        ...
	                        <div>
	                            ...
	                            <p>{{ $stcPaymentClaimNumberValue ?: '-' }}</p>
	                        </div>

	                        <div>
	                            <label>Payment Date</label>
	                            <p>{{ $stcPaymentDateValue ?: '-' }}</p>
	                        </div>
                    </div>
                    ...
                    <div class="rk-summary-row">
	                        <div>
	                            <label>Amount</label>
	                            <p>{{ $stcPaymentAmountValue !== '' ? '$'.$stcPaymentAmountValue : '-' }}</p>
	                        </div>
...
<div class="offcanvas offcanvas-end rk-vic-modal" id="vicWorkflowModal" tabindex="-1" aria-labelledby="vicWorkflowModalLabel">
            <div class="offcanvas-header rk-vic-modal-header">
                <div>
                    ...
                    <h4 class="offcanvas-title" id="vicWorkflowModalLabel">STCs Payment - Job #{{ $lead->id }}</h4>
            ...
            <div class="offcanvas-body rk-vic-modal-body">
                ...
                <div class="rk-vic-pane active" data-vic-pane="action">
                    <div class="rk-vic-info-card">
                        ...
                        <div>
                            <h5>STCs Payment Action Data</h5>
                    ...
                    <div class="row g-3">
                        <div class="col-md-12">
                            <div class="rk-vic-field-group mb-0">
                                <label for="stcPaymentStatus">Status</label>
                                <select id="stcPaymentStatus" class="form-select rk-vic-select-focus">
	                    ...
	                    <div class="row g-3">
	                        <div class="col-md-6">
	                            <div class="rk-vic-field-group mb-0">
	                                <label for="stcPaymentClaimNumber">Claim Number</label>
	                                <input type="text" id="stcPaymentClaimNumber" class="form-control" value="{{ $stcPaymentClaimNumberValue }}" placeholder="Enter STCs claim number">
	                            </div>
	                        </div>
	                        <div class="col-md-6">
	                            <div class="rk-vic-field-group mb-0">
	                                <label for="stcPaymentDate">Payment Date</label>
	                                <input type="date" id="stcPaymentDate" class="form-control" value="{{ $stcPaymentDateValue }}">
	                            </div>
	                        </div>
	                        <div class="col-md-12">
	                            <div class="rk-vic-field-group mb-0">
	                                <label for="stcPaymentAmount">Payment Amount</label>
	                                <input type="number" min="0" step="0.01" id="stcPaymentAmount" class="form-control" value="{{ $stcPaymentAmountValue }}" placeholder="0.00">
	                            </div>
	                        </div>
	                        <div class="col-md-12">
	                            <div class="rk-vic-field-group mb-0">
	                                <label for="stcPaymentNotes">Notes</label>
	                                <textarea id="stcPaymentNotes" class="form-control rk-vic-textarea" rows="3" placeholder="Add STCs payment notes">{{ $stcPaymentNotesValue }}</textarea>
	                            </div>
	                        </div>
	                    </div>
                ...
                <div class="rk-vic-pane" data-vic-pane="uploads">
                    <div class="rk-vic-info-card">
                        ...
                        <div>
	                            <h5>Uploads Data</h5>
	                            <p>Upload and review STCs payment receipts, claim files, and supporting documents.</p>
                        </div>
                    </div>

                    <div class="rk-vic-field-group">
	                        <label for="stcPaymentFileInput">Upload STCs Payment File</label>
	                        <div class="d-flex flex-wrap align-items-center gap-2">
	                            <input type="file" id="stcPaymentFileInput" class="form-control" style="max-width: 360px;">
	                            <button type="button" class="btn btn-primary" id="stcPaymentUploadBtn">
	                                <i class="ph ph-upload-simple"></i> Upload
	                            </button>
	                        </div>
	                        <small class="text-muted stc-payment-upload-status">No file selected</small>
                    </div>

	                    <ul class="rk-attach-list mt-3" id="stcPaymentFileList">
                        @forelse($lead->images as $file)
                            @php
                            $fileUrl = preg_replace('/^admin\//', '', $file->image_path);
                            $fullUrl = asset($fileUrl);
                ...
                <div class="rk-vic-pane" data-vic-pane="timeline">
                    <div class="rk-vic-info-card">
                        ...
                        <div>
                            <h5>Timeline</h5>
	                            <p>Review STCs payment notes and add the next claim update from this popup.</p>
                        </div>
                    </div>
                    ...
                    <form method="POST" action="{{ route('superadmin.noteStore') }}" class="mb-3">
                        ...
                        <div class="rk-vic-field-group mb-2">
	                            <label for="stcPaymentTimelineNote">Add Timeline Note</label>
	                            <textarea id="stcPaymentTimelineNote" name="note" class="form-control rk-vic-textarea" rows="3" placeholder="Add STCs payment timeline update..." required></textarea>
                        </div>
            ...
            <div class="rk-vic-modal-footer">
                <button type="button" class="btn btn-light" data-bs-dismiss="offcanvas">Close</button>
	                <button type="button" class="btn btn-primary" id="saveVicWorkflowBtn">Save STCs Payment Data</button>
            </div>
</div>
...
$(function() {
    ...
    $('.rk-vic-step').on('click', function() {
	    ...
	    $('#saveVicWorkflowBtn').on('click', function() {
	        const $btn = $(this);
	        const status = $('#stcPaymentStatus').val();
	
	        if (!status) {
	            setVicPane('action');
	            showWorkflowAlert('warning', 'Required', 'Please select STCs payment status.');
	            return;
	        }
	        ...
	        $.ajax({
	            url: '{{ route("superadmin.saveStcPaymentMeta") }}',
	            type: 'POST',
	            data: {
	                _token: '{{ csrf_token() }}',
	                lead_id: leadId,
	                status: status,
	                claim_number: $('#stcPaymentClaimNumber').val(),
	                payment_date: $('#stcPaymentDate').val(),
	                payment_amount: $('#stcPaymentAmount').val(),
	                notes: $('#stcPaymentNotes').val()
	            },
	            success: function() {
	                reloadWithSuccess('STCs payment data saved successfully.');
	            },
	            error: function(xhr) {
	                showWorkflowAlert('error', 'Failed', xhr.responseJSON && xhr.responseJSON.message ? xhr.responseJSON.message : 'Failed to save STCs payment data.');
	                $btn.prop('disabled', false).text('Save STCs Payment Data');
	            }
	        });
	    });
	
	    $('#stcPaymentFileInput').on('change', function() {
	        const file = this.files[0];
	        $('.stc-payment-upload-status').text(file ? file.name : 'No file selected');
	    });
	
	    $('#stcPaymentUploadBtn').on('click', function() {
	        const $btn = $(this);
	        const fileInput = $('#stcPaymentFileInput')[0];
	        const file = fileInput.files[0];
	
	        if (!file) {
	            setVicPane('uploads');
	            showWorkflowAlert('warning', 'Required', 'Please choose an STCs payment file to upload.');
	            return;
	        }
        ...
        const formData = new FormData();
	        ...
	        formData.append('status', $('#stcPaymentStatus').val());
	        formData.append('upload_context', 'stc_payment');
        ...
        $.ajax({
            ...
            processData: false,
	            ...
	            success: function(res) {
	                $('#stcPaymentFileList .rk-attach-empty').remove();
	                $('#stcPaymentFileList').append(
	                    '<li class="rk-attach-item"><a href="' + res.file_url + '" target="_blank" class="rk-attach-link"><i class="ph ph-file"></i> ' + res.file_name + '</a></li>'
	                );
	                fileInput.value = '';
	                $('.stc-payment-upload-status').text('Uploaded successfully');
	            },
	            error: function(xhr) {
	                showWorkflowAlert('error', 'Upload failed', xhr.responseJSON && xhr.responseJSON.message ? xhr.responseJSON.message : 'Failed to upload STCs payment file.');
	            },
            complete: function() {
