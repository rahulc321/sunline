...
$lead->status = $lead->status == 'Sold' ? 'COES AWAITING' : $lead->status;
...
$coesCertificateNumberValue = $leadMeta['coes_certificate_number'] ?? '';
$coesReceivedDateValue = $leadMeta['coes_received_date'] ?? '';
$coesInspectorNameValue = $leadMeta['coes_inspector_name'] ?? '';
$coesNotesValue = $leadMeta['coes_notes'] ?? '';
$vicWorkflowStatuses = [
...
<div class="rk-overview-wrapper">
    ...
    <div class="rk-overview-header">
        <div class="rk-overview-title-wrap">
            ...
            <div>
                <span class="rk-overview-kicker"><i class="ph ph-file-check"></i> Awaiting CoES</span>
        ...
        <div class="rk-column-stack rk-column-stack-full">
            <div class="row g-3">
                ...
                <div class="col-sm-6">
                    <div class="rk-summary-card h-100">
                <div class="rk-summary-header">
                    <strong><i class="ph ph-file-check"></i> CoES Details</strong>
                </div>

                <div class="rk-summary-content">
                    <div class="rk-summary-row">
                        <div>
                            <label>CoES Status</label>
                            <p>{{ $vicWorkflowStatuses[$currentVicWorkflowStatus] ?? $currentStatus }}</p>
                        </div>

	                        <div>
	                            <label>Certificate No</label>
	                            <p>{{ $coesCertificateNumberValue ?: '-' }}</p>
	                        </div>

	                        <div>
	                            <label>Received Date</label>
	                            <p>{{ $coesReceivedDateValue ?: '-' }}</p>
	                        </div>
                    </div>
...
<div class="offcanvas offcanvas-end rk-vic-modal" id="vicWorkflowModal" tabindex="-1" aria-labelledby="vicWorkflowModalLabel">
            <div class="offcanvas-header rk-vic-modal-header">
                <div>
                    <span class="rk-vic-offcanvas-kicker"><i class="ph ph-file-check"></i> Workflow</span>
                    <h4 class="offcanvas-title" id="vicWorkflowModalLabel">Awaiting CoES - Job #{{ $lead->id }}</h4>
            ...
            <div class="offcanvas-body rk-vic-modal-body">
                ...
                <div class="rk-vic-pane active" data-vic-pane="action">
                    <div class="rk-vic-info-card">
                        ...
                        <div>
                            <h5>CoES Action Data</h5>
                            <p>Update certificate status, capture CoES details, and run the next action from this popup.</p>
                        </div>
                    </div>

                    <h5 class="mb-3"><i class="ph ph-file-check"></i> CoES Progress</h5>
                    <div class="row g-3">
                        <div class="col-md-12">
                            <div class="rk-vic-field-group mb-0">
                                <label for="coesStatus">Status</label>
                                <select id="coesStatus" class="form-select rk-vic-select-focus">
	                    ...
	                    <h5 class="mb-3"><i class="ph ph-clipboard-text"></i> Certificate Data</h5>
	                    <div class="row g-3">
	                        <div class="col-md-6">
	                            <div class="rk-vic-field-group mb-0">
	                                <label for="coesCertificateNumber">Certificate Number</label>
	                                <input type="text" id="coesCertificateNumber" class="form-control" value="{{ $coesCertificateNumberValue }}" placeholder="Enter CoES number">
	                            </div>
	                        </div>
	                        <div class="col-md-6">
	                            <div class="rk-vic-field-group mb-0">
	                                <label for="coesReceivedDate">Received Date</label>
	                                <input type="date" id="coesReceivedDate" class="form-control" value="{{ $coesReceivedDateValue }}">
	                            </div>
	                        </div>
	                        <div class="col-md-12">
	                            <div class="rk-vic-field-group mb-0">
	                                <label for="coesInspectorName">Inspector / Electrician</label>
	                                <input type="text" id="coesInspectorName" class="form-control" value="{{ $coesInspectorNameValue }}" placeholder="Enter inspector or electrician name">
	                            </div>
	                        </div>
	                        <div class="col-md-12">
	                            <div class="rk-vic-field-group mb-0">
	                                <label for="coesNotes">Notes</label>
	                                <textarea id="coesNotes" class="form-control rk-vic-textarea" rows="3" placeholder="Add CoES notes">{{ $coesNotesValue }}</textarea>
	                            </div>
	                        </div>
	                    </div>
                ...
                <div class="rk-vic-pane" data-vic-pane="uploads">
                    <div class="rk-vic-info-card">
                        ...
                        <div>
	                            <h5>Uploads Data</h5>
	                            <p>Upload and review CoES certificates, electrical safety documents, and supporting files.</p>
                        </div>
                    </div>

                    <div class="rk-vic-field-group">
	                        <label for="coesFileInput">Upload CoES File</label>
	                        <div class="d-flex flex-wrap align-items-center gap-2">
	                            <input type="file" id="coesFileInput" class="form-control" style="max-width: 360px;">
	                            <button type="button" class="btn btn-primary" id="coesUploadBtn">
	                                <i class="ph ph-upload-simple"></i> Upload
	                            </button>
	                        </div>
	                        <small class="text-muted coes-upload-status">No file selected</small>
                    </div>

	                    <ul class="rk-attach-list mt-3" id="coesFileList">
                        @forelse($lead->images as $file)
                            @php
                            $fileUrl = preg_replace('/^admin\//', '', $file->image_path);
                            $fullUrl = asset($fileUrl);
                ...
                <div class="rk-vic-pane" data-vic-pane="timeline">
                    <div class="rk-vic-info-card">
                        ...
                        <div>
                            <h5>Timeline</h5>
	                            <p>Review CoES notes and add the next certificate update from this popup.</p>
                        </div>
                    </div>
                    ...
                    <form method="POST" action="{{ route('superadmin.noteStore') }}" class="mb-3">
                        ...
                        <div class="rk-vic-field-group mb-2">
	                            <label for="coesTimelineNote">Add Timeline Note</label>
	                            <textarea id="coesTimelineNote" name="note" class="form-control rk-vic-textarea" rows="3" placeholder="Add CoES timeline update..." required></textarea>
                        </div>
            ...
            <div class="rk-vic-modal-footer">
                <button type="button" class="btn btn-light" data-bs-dismiss="offcanvas">Close</button>
	                <button type="button" class="btn btn-primary" id="saveVicWorkflowBtn">Save CoES Data</button>
            </div>
</div>
...
$(function() {
    ...
    $('.rk-vic-step').on('click', function() {
	    ...
	    $('#saveVicWorkflowBtn').on('click', function() {
	        const $btn = $(this);
	        const status = $('#coesStatus').val();
	
	        if (!status) {
	            setVicPane('action');
	            showWorkflowAlert('warning', 'Required', 'Please select CoES status.');
	            return;
	        }
	        ...
	        $.ajax({
	            url: '{{ route("superadmin.saveCoesMeta") }}',
	            type: 'POST',
	            data: {
	                _token: '{{ csrf_token() }}',
	                lead_id: leadId,
	                status: status,
	                certificate_number: $('#coesCertificateNumber').val(),
	                received_date: $('#coesReceivedDate').val(),
	                inspector_name: $('#coesInspectorName').val(),
	                notes: $('#coesNotes').val()
	            },
	            success: function() {
	                reloadWithSuccess('CoES data saved successfully.');
	            },
	            error: function(xhr) {
	                showWorkflowAlert('error', 'Failed', xhr.responseJSON && xhr.responseJSON.message ? xhr.responseJSON.message : 'Failed to save CoES data.');
	                $btn.prop('disabled', false).text('Save CoES Data');
	            }
	        });
	    });
	
	    $('#coesFileInput').on('change', function() {
	        const file = this.files[0];
	        $('.coes-upload-status').text(file ? file.name : 'No file selected');
	    });
	
	    $('#coesUploadBtn').on('click', function() {
	        const $btn = $(this);
	        const fileInput = $('#coesFileInput')[0];
	        const file = fileInput.files[0];
	
	        if (!file) {
	            setVicPane('uploads');
	            showWorkflowAlert('warning', 'Required', 'Please choose a CoES file to upload.');
	            return;
	        }
        ...
        const formData = new FormData();
	        ...
	        formData.append('status', $('#coesStatus').val());
	        formData.append('upload_context', 'coes');
        ...
        $.ajax({
            ...
            processData: false,
	            ...
	            success: function(res) {
	                $('#coesFileList .rk-attach-empty').remove();
	                $('#coesFileList').append(
	                    '<li class="rk-attach-item"><a href="' + res.file_url + '" target="_blank" class="rk-attach-link"><i class="ph ph-file"></i> ' + res.file_name + '</a></li>'
	                );
	                fileInput.value = '';
	                $('.coes-upload-status').text('Uploaded successfully');
	            },
	            error: function(xhr) {
	                showWorkflowAlert('error', 'Upload failed', xhr.responseJSON && xhr.responseJSON.message ? xhr.responseJSON.message : 'Failed to upload CoES file.');
	            },
            complete: function() {
