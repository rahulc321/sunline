...
function fetchUnreadReplies() {
    ...
    $.ajax({
        ...
        success: function(response) {
            ...
            $('#friTable tbody tr').each(function() {

                let row = $(this);

                // get id from first column
                let ticketId = row.find('td:eq(0)').text().trim();

                // subject column
                let subjectCell = row.find('td:eq(1)');

                // remove old dot
                subjectCell.find('.red-dot').remove();

                // find ticket in response
                let ticketData = tickets.find(t => t.id == ticketId);

               // if (ticketData && ticketData.unread_replies_count > 0) {
                    subjectCell.append('<span class="red-dot ms-2"></span>');
              //  }
...
setInterval(fetchUnreadReplies, 10000);
